...
def isExpert (a : List Nat) (n : Nat) : Bool :=
  (List.range n).all (fun x => a.contains x)

-- Checks if everyone is an expert
def checkIfEE (s : (List (List Nat))) : Bool :=
  s.all (fun x => isExpert x s.length)
...
lemma inductive_case : ∀ (k : Nat), (Nat.succ k + 4 ≥ 4) → (∃ seq, checkIfEE (applyCalls (generateState (k + 4)) seq) = true) → ∃ seq', checkIfEE (applyCalls (generateState (Nat.succ k + 4)) seq') = true := by
  ...
  -- There is a sequence such that all agents except k know all secrets
  have h_expert_all : ∃ (seq''': List (Nat × Nat)), ∀ x, x < k + 4 → isExpert ((applyCalls (generateState (k + 4)) seq''').get! x) k := by
  ...
  -- After the final to last call, all agents except k know all secrets (maybe not necessary?)
